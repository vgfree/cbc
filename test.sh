...
check_cbc	"$noval"	"| bar | bar,"
check_cbc	"$noval"	"| bar |"
# checks (control-flow)
check_cbc	"0"			"| foo | foo := 10, while foo > 0 do foo := foo -1, end, foo,"
check_cbc	"5"			"| foo | foo := 10, if foo > 0 then 5, else 1, endif,"
check_cbc	"1"			"| foo | foo := 0, if foo > 0 then 5, else 1, endif,"
...
check_cbc	"False"		"| foo | foo := 'foobarbaz', foo <> 'foobarbaz',"
check_cbc	"True"		"| foo | foo := 'foobarbaz', foo <> 'foobar',"
