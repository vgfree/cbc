                                ...
                                bool check_result)
{
    ...
    if (check_result)
    {
        ...
        CuAssertIntEquals(tc, cb_value_get_type(expected_result),
                          cb_value_get_type(cb->result));
        // check execution result value
        switch (cb_value_get_type(cb->result))
        {
            case CB_VT_BOOLEAN:
                CuAssertIntEquals(tc, cb_boolean_get(expected_result), 
                                  cb_boolean_get(cb->result));
                break;
            
            case CB_VT_NUMERIC:
                CuAssertIntEquals(tc, cb_numeric_get(expected_result),
                                  cb_numeric_get(cb->result));
                break;
            
            case CB_VT_STRING:
                CuAssertStrEquals(tc, cb_string_get(expected_result),
                                  cb_string_get(cb->result));
                break;
